...
const ZXYStream = require('./zxystream');
const queue = require('d3-queue').queue;
const os = require('os');
const utils = require('./utils.js');

let stats = { max:0, total:0, count:0 };
...
function MBTiles(uri, callback) {
    ...
    const mode = flagEnum[uri.query.mode];
    if (!mode) {
...
MBTiles.list = function(filepath, callback) {
    ...
    fs.readdir(filepath, function(err, files) {
        ...
        for (let i = 0; i < files.length; i++) {
            const name = files[i].match(/^([\w-]+)\.mbtiles$/);
...
MBTiles.findID = function(filepath, id, callback) {
    filepath = path.resolve(filepath);
    const file = path.join(filepath, id + '.mbtiles');
...
MBTiles.prototype._exists = function(table, callback) {
    ...
    const sql = 'SELECT name FROM sqlite_master WHERE type IN ("table", "view")';
    const mbtiles = this;
...
MBTiles.prototype.getGrid = function(z, x, y, callback) {
    ...
    const sqlgrid = 'SELECT grid FROM grids WHERE zoom_level = ? AND tile_column = ? AND tile_row = ?';
    const sqljson = 'SELECT key_name, key_json FROM grid_data WHERE zoom_level = ? AND tile_column = ? AND tile_row = ?';
    ...
    mbtiles._db.get(sqlgrid, z, x, y, function(err, row) {
        ...
        zlib.inflate(!Buffer.isBuffer(row.grid) ? Buffer.from(row.grid, 'binary') : row.grid, function(err, buffer) {
            if (err) return callback(new Error('Grid is invalid:' + err.message));
            let grid;
            try { grid = JSON.parse(buffer); }
...
MBTiles.prototype.getInfo = function(callback) {
    ...
    mbtiles._db.all('SELECT name, value FROM metadata', function(err, rows) {
        ...
        if (rows) rows.forEach(function(row) {
            ...
            case 'json': {
                let jsondata;
                try { jsondata = JSON.parse(row.value); }
    ...
    function ensureZooms(info, callback) {
        ...
        let remaining = 30;
        const zooms = [];
        const query = mbtiles._db.prepare('SELECT zoom_level FROM tiles WHERE zoom_level = ? LIMIT 1', function(err) {
...
MBTiles.prototype.write = function(table, id, row, callback) {
    ...
    // Merge row data.
    for (let key in row) this._writes[table][id][key] = row[key];
    ...
    mbtiles._db.serialize(function() {
        ...
        Object.keys(writes).forEach(function(table) {
            ...
            case 'map': {
                ...
                for (let id in writes[table]) {
                    const row = writes[table][id];
                    statements['map'].run(
            ...
            default: {
                ...
                const sql = 'REPLACE INTO ' + table + ' ( ' + fields.join(',') + ' ) VALUES(' + args.join(',') + ')';
...
MBTiles.prototype.putGrid = function(z, x, y, data, callback) {
    ...
    const json = JSON.stringify({ grid: data.grid, keys: data.keys });
    ...
    zlib.deflate(Buffer.from(json, 'utf8'), function(err, buffer) {
        if (err) return callback(err);
...
MBTiles.prototype.geocoderDataIterator = function(type) {
    const chunkSize = 100;
    let position = 0;
    const getNextIfBelow = 0.2 * chunkSize;
    const nextQueue = [];
    const dataQueue = [];
    const doneSentinel = {};
    const _this = this;

    const zlibQueue = queue(1);
    ...
    let sending = false;
    const sendIfAvailable = function() {
        if (sending) return;
        sending = true;

        while (nextQueue.length && dataQueue.length) {
            const nextCb = nextQueue.shift();
            let data;
    ...
    const maybeRefillBuffer = function() {
...
MBTiles.prototype.geocoderMigrateDocs = function(rows, callback) {
    ...
    function tms2zxy(zxys) {
        return zxys.split(',').map(function(tms) {
            const zxy = tms.split('/').map(function(v) { return parseInt(v, 10); });
    ...
    const doc = JSON.parse(row.key_json);
    const text = row.text || doc.search || doc.name || '';
    if ('zxy' in row && text) {
...
MBTiles.prototype.geocoderCentroid = function(id, zxy, callback) {
    const coords = [];
    for (let i = 0; i < zxy.length; i++) {
        const parts = zxy[i].split('/');
        parts[0] = parts[0] | 0;
        parts[1] = parts[1] | 0;
        parts[2] = parts[2] | 0;
        coords.push(parts);
    ...
    const mid = coords[Math.floor(coords.length * 0.5)];
    this.getGrid(mid[0],mid[1],mid[2],function(err, grid) {
        ...
        const cxy = xy[Math.floor(xy.length * 0.5)];
        callback(null, sm.ll([
            (256*mid[1]) + (cxy[0]*4),
            (256*mid[2]) + (cxy[1]*4)
        ], mid[0]));
